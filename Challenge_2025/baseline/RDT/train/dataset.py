...
class VLAConsumerDataset(Dataset):
    ...
    def __init__(
        ...
        dataset_names_cfg = (
            "configs/pretrain_datasets.json" if dataset_type == "pretrain" else "configs/finetune_datasets.json"
        )
        ...
        if use_hdf5:
            ...
            self.hdf5_dataset = HDF5A2DDataset(use_tasks=use_tasks)
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                ...
                data_dict["dataset_name"] = "aw_sim"
                data_dict["data_idx"] = self.dataset_name2id[data_dict["dataset_name"]]
                data_dict["ctrl_freq"] = (
                    self.control_freq[data_dict["dataset_name"]] if random.random() > self.cond_mask_prob else 0
                )

                if self.state_noise_snr is not None:
                    states += np.random.normal(
                        0.0, state_std / np.sqrt(10 ** (self.state_noise_snr / 10)), states.shape
                    )
                #if self.dataset_type == "finetune" and not self.use_tasks == "all":
                #    if self.use_tasks == "697u":
                #        ds_state_mean = np.array(self.dataset_stat[data_dict["dataset_name"] + "_697"]["state_mean"])
                #    else:
                #        ds_state_mean = np.array(
                #            self.dataset_stat[data_dict["dataset_name"] + "_" + self.use_tasks]["state_mean"]
                #        )
                #else:
                ...
                data_dict["state_elem_mask"] = (
                    state_elem_mask if random.random() > self.cond_mask_prob else np.zeros_like(state_elem_mask)
                )
                ...
                for image, valid in rearranged_images:
                    ...
                    if valid and self.image_aug and (random.random() > 0.5):
                        ...
                        if aug_type != "corrput_only":
                            image = transforms.ColorJitter(brightness=0.3, contrast=0.4, saturation=0.5, hue=0.03)(
                                image
                            )
                ...
                if self.use_precomp_lang_embed:
                    ...
                    data_dict["lang_embed"] = (
                        torch.load(content["instruction"])
                        if random.random() > self.cond_mask_prob
                        else self.empty_lang_embed
                    )
                else:
