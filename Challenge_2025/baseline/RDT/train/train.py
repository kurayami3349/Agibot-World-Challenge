...
def train(args, logger):
    ...
    if args.pretrained_model_name_or_path is not None and not os.path.isfile(args.pretrained_model_name_or_path):
        ...
        img_cond_len = (
            config["common"]["img_history_size"] * config["common"]["num_cameras"] * vision_encoder.num_patches
        )
        rdt = RDTRunner(
            ...
            with accelerator.accumulate(rdt):
                ...
                with torch.no_grad():
                    ...
                    text_embeds = (
                        ...
                        if args.precomp_lang_embed
                        else text_encoder(input_ids=batch["input_ids"], attention_mask=lang_attn_mask)[
                            "last_hidden_state"
                        ].detach()
                    )
                ...
                if global_step % args.checkpointing_period == 0:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            logger.info(f"[ERROR] saving checkpoint on attempt {attempt + 1}")
                            logger.info(e)
