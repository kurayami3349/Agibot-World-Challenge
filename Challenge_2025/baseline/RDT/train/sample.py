...
@torch.no_grad()
def log_sample_res(
    text_encoder, vision_encoder, rdt, args, accelerator, weight_dtype, dataset_id2name, dataloader, logger
):
    ...
    for step, batch in enumerate(dataloader):
        ...
        text_embeds = (
            ...
            else text_encoder(input_ids=batch["input_ids"], attention_mask=lang_attn_mask)["last_hidden_state"].detach()
        )
        ...
        mse_loss_per_entry = (loss * expanded_state_elem_mask).reshape((batch_size, -1)).sum(
            1
        ) / expanded_state_elem_mask.reshape((batch_size, -1)).sum(1)
        l2_loss_per_entry = loss.sqrt() / (expanded_state_norm + 1e-3)
        l2_loss_per_entry = (l2_loss_per_entry * expanded_state_elem_mask).reshape((batch_size, -1)).sum(
            1
        ) / expanded_state_elem_mask.reshape((batch_size, -1)).sum(1)

        dataset_indices, mse_losses, l2_losses = accelerator.gather_for_metrics(
            (torch.LongTensor(data_indices).to(device=pred_actions.device), mse_loss_per_entry, l2_loss_per_entry),
        )
    ...
    for name in loss_for_log:
        if name in ["overall_avg_sample_mse", "overall_avg_sample_l2err"]:
            ...
            loss_for_log[name] = round(loss_scaler / (args.num_sample_batches), 6)
        else:
            loss_for_log[name] = round(loss_for_log[name] / loss_counter[name], 6)
