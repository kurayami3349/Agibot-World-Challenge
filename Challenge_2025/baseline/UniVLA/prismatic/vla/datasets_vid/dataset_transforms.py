...
@contextmanager
def open_video_container(video_path: str):
    """视频容器上下文管理器"""
    container = None
    try:
    ...
    with open_video_container(video_path) as container:
        ...
        for idx in indices:
            try:
                ...
                print(f"Error seeking to frame {idx}: {e}")
                frames.append(None)
    ...
    def mp_worker(self, info):
        ...
        try:
            ...
            for act_step in action_config:
                ...
                stop_idx = act_step["end_frame"]
                try:
                    job_description = info["task_name"]
                    sub_job_description = act_step["action_text"]
                except:
                    job_description = ""
                    sub_job_description = ""
            ...
            logger.error(f'task:{info["task_id"]},episode:{info["episode_id"]} met error: {error}')
            results = {}
        return results
...
class RuntimeVideoPreprocessLoad:
    ...
    def __call__(self, inputs):
        ...
        try:
            for name in self.main_key:
                video_path = os.path.join(inputs["evdir"], "head_color.mp4")
                video_data = get_frames_by_indices(
                    video_path=video_path,
                    indices=[int(inputs["frame_idx"]), int(inputs["target_idx"])],
                )
                
                img = Image.fromarray(video_data[0])
                img_k = Image.fromarray(video_data[1])
            ...
            logger.error(f'video:{inputs["evdir"]} met error: {error}')
        finally:
            # 清理资源
            if img is not None:
                img.close()
            ...
            if memory_diff > 100:  # 如果内存增长超过 100MB
                logger.warning(f"Memory usage increased by {memory_diff:.2f} MB, before: {memory_before:.2f} MB, after: {memory_after:.2f} MB")
